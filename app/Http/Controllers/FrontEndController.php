...
class FrontEndController extends JoshController
{
    ...
    public function myAccount(User $user)
    {
        ...
        $this->getDiffVal();
        return view('front.my-account', compact('user'));
    }
    ...
    {
        $user = Sentinel::getUser();
        $where = array("email"=> $user['email'], "password"=>$request->get("old_password"));
        if (!Sentinel::authenticate($where, false)){
            return Redirect::route('my-account')->withInput()->with('error', "The current password is no valid");
        }
